  ...
  detect-changes:
    ...
    steps:
      - uses: actions/checkout@v4
        with:
          # Fetch depth 0 is ESSENTIAL for git diff comparison
          fetch-depth: 0 
          
      - name: 🔍 Detect Changed Files in the PR
        uses: GrantBirki/git-diff-action@v2.8.1
        id: git-diff-action
        with:
          # Output the list of files to a specific file path
          files_only_output: changed_files.txt
          
      - name: ⚙️ Process Diff to Find SAM Template Directories
        id: process-diff
        shell: bash
        run: |
          ...
          echo "::set-output name=changed_dirs::$JSON_ARRAY"
...
#     # 🌟 Matrix Strategy: Iterate over only the detected changes
#     strategy:
